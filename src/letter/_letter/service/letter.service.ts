...
import { RecipientService } from '../../recipient/_recipient/service/recipient.service';
...
export class LetterService extends BaseService<Letter> {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => LetterApprovalService))
  ...
  async create(payload: LetterCreateRequest, tokenPayload: AccessTokenPayload) {
    ...
    const template = await this.templateService.getBy('id', templateId, {
      relations: { approvalDepartmentRoles: true, workflows: true },
    });
    ...
    if (workflows && workflows.length) hasWorkflow = true;

    const templateApprovalRoles = template.approvalDepartmentRoles;
    const letterNumber = generateLetterNumber();

    let newLetter = this.rep.create({ ...payload, number: letterNumber, creatorUserId: userId, hasWorkflow });
    newLetter = await this.rep.save(newLetter);

    if (templateApprovalRoles && templateApprovalRoles.length) {

...
describe('UserService', () => {
  ...
  const setDepartmentPayload: UserSetDepartmentRequest = { departmentId: 2 };
  ...
  describe('createUser', () => {
    it('should create a user successfully and return a UserDto', async () => {
      ...
      (CryptoUtils.hashPassword as jest.Mock).mockResolvedValue(
        'hashedpassword',
      );
      ...
      expect(userRepository.findOne).toHaveBeenCalledWith({
        where: { username: createUserPayload.username },
      });
      expect(CryptoUtils.hashPassword).toHaveBeenCalledWith(
        createUserPayload.password,
      );
      expect(departmentService.checkExistsById).toHaveBeenCalledWith(
        createUserPayload.departmentId,
      );
    ...
    it('should throw a ConflictException if the username already exists', async () => {
      ...
      // Act & Assert
      await expect(service.createUser(createUserPayload)).rejects.toThrow(
        ConflictException,
      );
      expect(userRepository.findOne).toHaveBeenCalledWith({
        where: { username: createUserPayload.username },
  ...
  describe('setDepartment', () => {
    it('should successfully update the user department', async () => {
      ...
      expect(departmentService.checkExistsById).toHaveBeenCalledWith(
        setDepartmentPayload.departmentId,
      );
    ...
    it('should throw a NotFoundException if the user to update does not exist', async () => {
      ...
      // Act & Assert
      await expect(
        service.setDepartment(99, setDepartmentPayload),
    ...
    it('should propagate an error if the new department does not exist', async () => {
      ...
      // Act & Assert
      await expect(
        service.setDepartment(mockUser.id, setDepartmentPayload),
